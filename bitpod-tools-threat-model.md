...
- In-scope paths:
  ...
  - `$WORKSPACE/bitpod-tools/costs`
  - `$WORKSPACE/bitpod-tools/audit_ctl.sh` (operational helper, low runtime criticality)
- Out-of-scope:
...
|---|---|---|---|---|
| `POST /ask` | Internet via domain/tunnel or local URL | External caller -> bridge service | Core auth and validation chokepoint | `$WORKSPACE/bitpod-tools/gpt_bridge/gpt_bridge.py` (`do_POST`, `_is_authorized`) |
| MCP tool `gpt_bridge_ask` | MCP stdio clients | MCP client -> bridge | Forwards args to `GPT_BRIDGE_URL`; auth header from env | `$WORKSPACE/bitpod-tools/gpt_bridge/gpt_bridge_mcp.py` (`_call_bridge`, `_build_headers`) |
| CLI wrapper `ask_gpt.py` | Local shell scripts/users | Operator shell -> bridge | Can post to remote bridge URL from env | `$WORKSPACE/bitpod-tools/gpt_bridge/ask_gpt.py` |
| Chat command relay | `bridge_chat.sh chat` / team command aliases | User text -> GPT relay + memory pipeline | Mention parsing and auto memory operations | `$WORKSPACE/bitpod-tools/gpt_bridge/bridge_chat.py` (`run_team`, `_finalize_session_memory`) |
| Log files | Local FS reads/writes | App process -> filesystem | Contains high-value transcript and trace content | `$WORKSPACE/bitpod-tools/gpt_bridge/gpt_bridge.py` (`_log_jsonl`), `bridge_chat.py` (`append_event`) |
...
|---|---|---|
| `$WORKSPACE/bitpod-tools/gpt_bridge/gpt_bridge.py` | Primary auth, request parsing, external API forwarding, logging | TM-001, TM-002, TM-003, TM-004 |
| `$WORKSPACE/bitpod-tools/gpt_bridge/gpt_bridge_mcp.py` | MCP->HTTP forwarding and auth header construction | TM-001, TM-006 |
| `$WORKSPACE/bitpod-tools/gpt_bridge/bridge_chat.py` | Chat relay, memory extraction/sync, persistent event logging | TM-004, TM-005 |
| `$WORKSPACE/bitpod-tools/gpt_bridge/bridge_ctl.sh` | Runtime control logic and remote/local endpoint behavior | TM-002, TM-006 |
| `$WORKSPACE/bitpod-tools/gpt_bridge/ask_gpt.py` | Client-side endpoint and auth usage path | TM-001, TM-006 |
| `$WORKSPACE/bitpod-tools/gpt_bridge/config.example.env` | Secret and endpoint configuration contract | TM-001, TM-006 |
| `$WORKSPACE/bitpod-tools/costs/cost_ctl.py` | Cost anomaly signal path for abuse detection | TM-003 |
