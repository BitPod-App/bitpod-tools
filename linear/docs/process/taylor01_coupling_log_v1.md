...
| T01-C13 | `/Users/cjarguello/.agents/skills/qa-specialist/**` | Home-directory skill coupling | A reusable QA lane still depends on a home-directory mirror path outside the canonical workspace root | Treat `local-codex` copy as canonical local surface and retire the mirror over time |
| T01-C15 | `/Users/cjarguello/.agents/skills/taylor/**` | Home-directory skill coupling | Taylor skill mirror still exists outside the canonical workspace root even though active repo surfaces now prefer `local-codex` | Keep `local-codex` as canonical local surface and retire the mirror once runtime compatibility is clear |
