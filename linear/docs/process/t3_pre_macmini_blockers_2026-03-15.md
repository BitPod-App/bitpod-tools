...
| Stale local Codex app metadata | `PASS_WITH_LIMITS` | `.codex-global-state.json` is still stale, but the current active workspace roots are already correct; the remaining drift is UI/app metadata now covered by `local_codex_app_state_tolerance_note_v1.md` and `local_codex_app_state_reset_plan_v1.md` | reset only in a fresh-session window if strict T3 cleanliness is required or the Mac Mini needs a clean exported app-state baseline |
